  ...
  mensajeConfirmacionTurno() {
    return alert(
      this.usuario +
        ...
        this.hora +
        " hs., hasta las " +
        convertirFloatAHora(this.horaFinTurno()) +
...
function usuarioValido(nombreUsuario, pass) {
  ...
  if (
    ...
    usuarioEncontrado.paswordCorrecto(pass)
  ) {
    return true;
  } else {
    return false;
...
function menuInicio() {
  alert(
    "Bienvenido a Jardín Padel Club!\nEn esta sección podrás solicitar y reservar un turno. \nRecuerda que si eres usuario registrado accedes a un descuento del " +
      DESCUENTOUSUARIOREGISTRADO * 100 +
      "%"
  );
  do {
    opcionInicio = prompt(
...
function telefonoEnSesion() {
  if (invitadoCorrecto) {
    ...
    return usuariosRegistrados.find((usu) => usu.nombre === nombreEnSesion())
      .telefono;
  } else {
    return "no informado";
  }
...
function disponible() {
  if (turnosConfirmados.length == 0) {
    ...
    turnosConfirmados.some((turno) => turno.fecha == fechaTurno()) &&
    turnosConfirmados.some((turno) => turno.hora == hora)
  ) {
    return false;
  } else {
    agregarTurnoConfirmado();
...
function ingresoDatosDelTurno() {
  ...
  do {
    duracion = Number(
      entradaValidaTurno("la duración (Eje.: 1.5 = hora y media)")
    );
